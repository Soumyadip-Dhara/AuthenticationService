...
using UserManagement.bff.Auth;
using UserManagement.bff.Endpoints;
...
.AddOpenIdConnect("oidc", options =>
{
    ...
    options.Scope.Add("api:usermanagement");
    ...
    options.SignedOutRedirectUri = builder.Configuration["FrontendUrl"] ?? "/";
...
// ──────────────────────────────────────────────
// 4. YARP Reverse Proxy — forwards /api/** to API
// ──────────────────────────────────────────────
...
// Health check
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "UserManagement.bff" }));
