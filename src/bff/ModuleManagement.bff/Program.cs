...
{
    options.Cookie.Name = ".modulemanagement.session";
...
.AddOpenIdConnect("oidc", options =>
{
    ...
    options.Authority = config["Authority"] ?? "https://10.176.100.90:5001";
    ...
    options.Scope.Add("api:modulemanagement");
    ...
    options.SignedOutRedirectUri = "https://10.176.100.90:4500/";
    ...
    options.Events = new OpenIdConnectEvents
    {
        ...
        OnTokenValidated = context =>
        {
            ...
            {
                // The session key will be set by the ticket store during StoreAsync
                // We need to hook into the cookie events to capture it
                // For now, store the sid in the properties so we can access it later
...
// ──────────────────────────────────────────────
// We wrap the InMemoryTicketStore to intercept StoreAsync
// and register the sid→sessionKey mapping in SidIndex.
...
// ──────────────────────────────────────────────
// 4. YARP Reverse Proxy — forwards /api/** to Demo.Api
// ──────────────────────────────────────────────
...
// ──────────────────────────────────────────────
// 5. CORS — allow Demo UI origin
// ──────────────────────────────────────────────
builder.Services.AddCors(options =>
{
    options.AddDefaultPolicy(policy =>
    {
        policy.WithOrigins("https://10.176.100.90:4500")
            .AllowAnyHeader()
            .AllowAnyMethod()
...
// Inject the access token from the server-side session into the upstream request
...
// Health check
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "AuthService.DemoBff" }));

app.Run();

/// <summary>
/// Wraps InMemoryTicketStore to capture the sid→sessionKey mapping
